...
runningapps(){
	...
	echo ">> No in use programs need updating.  Awesome!"
        ...
        result=`/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType hud -title "PEAS Updates" -hdeading "Programs in Use" -description "The applications ${RUNNINGAPPSARRAY[@]} need updating but are in use.  Please quit those programs and try again." -button2 "Skip" -button1 "Retry" -default button2`
        if [ "$result" == "0" ]; then
            checkForUpdates
        fi
    else
